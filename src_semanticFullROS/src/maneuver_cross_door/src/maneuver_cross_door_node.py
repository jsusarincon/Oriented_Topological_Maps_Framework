...
ANGLEPERRAY=0.40625 #260/640 #260 degrees is 640 rays from laser
...
def initVariables():
	global alignDone,facingDoor,pointingDoor,doorIndex,goDoor,middleOfDoor,angleMiddleDoor,distanceToWall,start,values,directionLastNode
	alignDone=0
	facingDoor=0	#robot facing the door
	pointingDoor=0	#robot with angle to cross door
	doorIndex=[0,0] #start and end of the door
...
def crossDoor(go):
	global directionLastNode,LINVELFACTOR,ANGVELFACTOR,alignDone,start
	print "goooo"
	

	if go.data!=0: #if the value is 0 this means the robot doesnt move and this maneuver is not activated
		start=1
		#the data can be from 1 to 8, indicating if the door is E,N,W or S, fast or slow
		if go.data==1 or go.data==2 or go.data==3 or go.data==4 :
			LINVELFACTOR=2
			ANGVELFACTOR=2

		if go.data==5 or go.data==6 or go.data==7 or go.data==8 :
			LINVELFACTOR=6
			ANGVELFACTOR=6

		if go.data==1 or go.data==5:
			directionLastNode=1
		elif go.data==2 or go.data==6:
			directionLastNode=2
		elif go.data==3 or go.data==7:
			directionLastNode=3
		elif go.data==4 or go.data==8:
			directionLastNode=4
...
def alignRobot(values):
	...
	#1) East goes from 355 to 5
	#2) North goes from 84 to 95
	#3) West goes from 175 to 185
	#4) South goes from 265 to 275

	if directionLastNode==1 and facingDoor==0: #facing east
	
		if directionRobotDeg >=355 or directionRobotDeg<=5:
			print "east", directionRobotDeg
			moveRobot(0.0,0.0) 
			facingDoor=1
		elif directionRobotDeg >5 and directionRobotDeg<=180 :
			moveRobot(0.0,-0.2*ANGVELFACTOR) #CW
		elif directionRobotDeg <355 and directionRobotDeg>180 :
			moveRobot(0.0,0.2*ANGVELFACTOR) #CCW

	elif directionLastNode==2 and facingDoor==0: #facing north
	
		if directionRobotDeg >=85 and directionRobotDeg<=95:
			print "north", directionRobotDeg
			moveRobot(0.0,0.0) 
			facingDoor=1
		elif directionRobotDeg >95 and directionRobotDeg<=270:
			moveRobot(0.0,-0.2*ANGVELFACTOR) #CW
		elif directionRobotDeg >270 or directionRobotDeg<85 :
			moveRobot(0.0,0.2*ANGVELFACTOR) #CCW


	elif directionLastNode==3 and facingDoor==0: #facing west

		if directionRobotDeg >=175 and directionRobotDeg<=185:
			print "west", directionRobotDeg
			moveRobot(0.0,0.0) 
			facingDoor=1
		elif directionRobotDeg <=360 and directionRobotDeg>185:
			moveRobot(0.0,-0.2*ANGVELFACTOR) #CW
		elif directionRobotDeg >0 and directionRobotDeg<175:
			moveRobot(0.0,0.2*ANGVELFACTOR) #CCW

	elif directionLastNode==4 and facingDoor==0: #facing south
	
		if directionRobotDeg >=265 and directionRobotDeg<=275:
			print "south", directionRobotDeg
			moveRobot(0.0,0.0) 
			facingDoor=1
		elif directionRobotDeg <= 90 or directionRobotDeg>=275:
			moveRobot(0.0,-0.2*ANGVELFACTOR) #CW
		elif directionRobotDeg >90 and directionRobotDeg<265:
			moveRobot(0.0,0.2*ANGVELFACTOR) #CCW
	...
	if facingDoor==1 and pointingDoor==0:
		...
		counter=0
		for i in range(300,340): #all the front has to be free
			if values[i]>5:
				counter+=1
		...
		if counter>=39:
			...
			#for i in range(210,430): #from -45 to 45, all the front check where the door is
				...
				if difference>errorAngleDoor and startDone==0: #if the difference bewteen 2 rays is more than 1m there is a hole
					...
					if directionLastNode==2 or directionLastNode==4:
						angleMiddleDoor=abs(middleOfDoor)*ANGLEPERRAY-40
						if directionRobotDeg>180:
							angleMiddleDoor+=180
					if directionLastNode==1 :
						angleMiddleDoor=abs(middleOfDoor)*ANGLEPERRAY-40
						if middleOfDoor>320:
							angleMiddleDoor=angleMiddleDoor-90
						else:
							angleMiddleDoor=360-(90-angleMiddleDoor)
					if directionLastNode==3:
						angleMiddleDoor=abs(middleOfDoor)*ANGLEPERRAY-40
						if middleOfDoor>320:
							angleMiddleDoor=angleMiddleDoor+90
						else:
							angleMiddleDoor=180-(90-angleMiddleDoor)
...
def stayCenter(values):
	...
	if centermean<DISTEMERGENCYSTOP:
		...
		mean60Left=sum(values[464:470])/len(values[464:470])
		mean60Right=sum(values[170:176])/len(values[170:176])
		mean45Left=sum(values[427:433])/len(values[427:433])
...
def readDirection(directionRobotMsg):
	...
	if directionRobotDeg<0:
		directionRobotDeg=360+directionRobotDeg
...
def listenerScan():
	...
	rospy.Subscriber("noisy_compass",Float64, readDirection)
	rospy.Subscriber("go_cross_door",Int16, crossDoor)	
	rospy.spin()
